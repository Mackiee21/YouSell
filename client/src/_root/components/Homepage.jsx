...
function Homepage() {
  ...
  return (
    <main className="flex flex-col gap-7">
      <section id="hero" className="flex rounded p-2 items-center h-[330px]">
          <div className="ms-14">
            <h1 className="logo border-y-2 border-teal-600 text-slate-600 w-fit tracking-wider font-bold font-mono text-3xl">YouSell</h1>
            <h2 className="mt-2">Have some pre-used books? Sell with YouSell</h2>
            <button className="btn-primary mt-5">Shop now</button>
          </div>
      </section> {/*HERO*/}
      <div className="flex items-center gap-2">
        <h1 className='text-teal-600 font-semibold text-xl logo tracking-wide'>Top Choices</h1>
        <span className="flex-1 h-0.5 bg-slate-200"></span>
      </div>
      {isLoading && <Skeleton howMany={10} /> }
      <section className={`grid grid-cols-4 xl:grid-cols-5 gap-2.5 md:gap-7 transition-all duration-1000 ${isLoading ? 'opacity-0' : 'opacity-100' }`}>
